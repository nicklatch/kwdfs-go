...
(
    ...
    state                   char(2)      not null,
    pfleet_acct_id          varchar(20)  not null,
    status                  fleet_status default 'national'::fleet_status,
    truck_qty               smallint     not null,
    fleet_support_rep       uuid         not null,
    field_service_rep       varchar(100),
    field_service_rep_email varchar(255)
);

comment on column customers.state is 'Two letter state abbr';

comment on column customers.pfleet_acct_id is 'Usually a 4 char length id';

comment on column customers.field_service_rep is 'Full name';
